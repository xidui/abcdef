...
for processing_day in range(22, 31):
	...
	#去除重复等处理  只统计order supply gap
	a_deduped=a.groupby(['passenger_id','start_district_hash','dest_district_hash','Hour','Minute','Day','time_id']).count().reset_index()
	a_deduped=a_deduped.rename(columns={'driver_id':'Supply_cnt_deduped','order_id':'Order_cnt_deduped'})
	a_deduped['Gap_cnt_deduped']=a_deduped.Order_cnt_deduped-a_deduped.Supply_cnt_deduped
	a_deduped=a_deduped.groupby(['start_district_hash','Day','time_id']).sum().reset_index()
	a_deduped=a_deduped['start_district_hash','Day','time_id','Order_cnt_deduped','Supply_cnt_deduped','Gap_cnt_deduped']

	
	b=pd.DataFrame(columns=a_deduped.columns)
	for now_time_id in range(43,145):
		temp_table=a_deduped[[tmp in range(now_time_id-1,now_time_id) for tmp in a_deduped.time_id]]
		temp_table.loc[:,['time_id']]=now_time_id
		b=b.append(temp_table)
	b=b.rename(columns={'Order_cnt_deduped':'Order_cnt_deduped_10min','Supply_cnt_deduped':'Supply_cnt_deduped_10min','Gap_cnt_deduped':'Gap_cnt_deduped_10min'})
	mother_table=pd.merge(mother_table,b,on=['start_district_hash','Day','time_id'],how='left')
	
	b=pd.DataFrame(columns=a_deduped.columns)
	for now_time_id in range(43,145):
		temp_table=a_deduped[[tmp in range(now_time_id-2,now_time_id-1) for tmp in a_deduped.time_id]]
		temp_table.loc[:,['time_id']]=now_time_id
		b=b.append(temp_table)
	b=b.rename(columns={'Order_cnt_deduped':'Order_cnt_deduped_20min','Supply_cnt_deduped':'Supply_cnt_deduped_20min','Gap_cnt_deduped':'Gap_cnt_deduped_20min'})
	mother_table=pd.merge(mother_table,b,on=['start_district_hash','Day','time_id'],how='left')

	b=pd.DataFrame(columns=a_deduped.columns)
	for now_time_id in range(43,145):
		temp_table=a_deduped[[tmp in range(now_time_id-3,now_time_id-2) for tmp in a_deduped.time_id]]
		temp_table.loc[:,['time_id']]=now_time_id
		b=b.append(temp_table)
	b=b.rename(columns={'Order_cnt_deduped':'Order_cnt_deduped_30min','Supply_cnt_deduped':'Supply_cnt_deduped_30min','Gap_cnt_deduped':'Gap_cnt_deduped_30min'})
	mother_table=pd.merge(mother_table,b,on=['start_district_hash','Day','time_id'],how='left')	
	...
	#calculate number of gap decreased by dedupe
	for base_name in ['Order_cnt','Supply_cnt','Gap_cnt']:
		for suffix in ['_10min','_20min','_30min']:
			cname=base_name+'_deduped'+suffix
            new_cname=cname+'_duplicate'
			mother_table[new_cname]=mother_table[base_name+suffix]-mother_table[cname]	
	#--------------------------------------------------------------------------------------------------------------------
	...
	if os.path.exists(action_file):
		...
		mother_table=Order_Supply_Gap_attribute(a,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_')
        ...
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_10Min_')
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['Weekday','time_id'],'Weekday_10Min_AllDistrict_')
        swap_start_dest(b)
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_Dest_10Min_')
        swap_start_dest(b)
        
        b_not_null=b[b.driver_id.notnull()]
        mother_table=price_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','Price'],['start_district_hash','Weekday','time_id'],'Weekday_10Min_Day_')
        mother_table=price_attribute(b_not_null,mother_table,['Weekday','time_id','Price'],['Weekday','time_id'],'Weekday_10Min_AllDistrict_Day_')

        mother_table=distinct_driver_attribute(b_not_null,mother_table,['Weekday','time_id','driver_id'],['Weekday','time_id'],'Weekday_10Min_AllDistrict_')
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_10Min_AllDistrict_AllOrder_')
        mother_table=distinct_passenger_attribute(b,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_10Min_AllOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_10Min_AllDistrict_AcceptOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_10Min_AcceptOrder_')
        swap_start_dest(b)
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_10Min_Desti_AllOrder_')
        swap_start_dest(b)
        swap_start_dest(b_not_null)
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_10Min_Desti_AcceptOrder_')
        swap_start_dest(b_not_null)
        ...
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_20Min_')
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['Weekday','time_id'],'Weekday_20Min_AllDistrict_')
        swap_start_dest(b)
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_Dest_20Min_')
        swap_start_dest(b)
        
        b_not_null=b[b.driver_id.notnull()]
        mother_table=price_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','Price'],['start_district_hash','Weekday','time_id'],'Weekday_20Min_Day_')
        mother_table=price_attribute(b_not_null,mother_table,['Weekday','time_id','Price'],['Weekday','time_id'],'Weekday_20Min_AllDistrict_Day_')

        mother_table=distinct_driver_attribute(b_not_null,mother_table,['Weekday','time_id','driver_id'],['Weekday','time_id'],'Weekday_20Min_AllDistrict_')
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_20Min_AllDistrict_AllOrder_')
        mother_table=distinct_passenger_attribute(b,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_20Min_AllOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_20Min_AllDistrict_AcceptOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_20Min_AcceptOrder_')
        swap_start_dest(b)
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_20Min_Desti_AllOrder_')
        swap_start_dest(b)
        swap_start_dest(b_not_null)
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_20Min_Desti_AcceptOrder_')
        swap_start_dest(b_not_null)
        ...
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_30Min_')
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['Weekday','time_id'],'Weekday_30Min_AllDistrict_')
        swap_start_dest(b)
        mother_table=Order_Supply_Gap_attribute(b,mother_table,['start_district_hash','Weekday','time_id'],'Weekday_Dest_30Min_')
        swap_start_dest(b)

        b_not_null=b[b.driver_id.notnull()]
        mother_table=price_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','Price'],['start_district_hash','Weekday','time_id'],'Weekday_30Min_Day_')
        mother_table=price_attribute(b_not_null,mother_table,['Weekday','time_id','Price'],['Weekday','time_id'],'Weekday_30Min_AllDistrict_Day_')

        mother_table=distinct_driver_attribute(b_not_null,mother_table,['Weekday','time_id','driver_id'],['Weekday','time_id'],'Weekday_30Min_AllDistrict_')
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_30Min_AllDistrict_AllOrder_')
        mother_table=distinct_passenger_attribute(b,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_30Min_AllOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_30Min_AllDistrict_AcceptOrder_')
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['start_district_hash','Weekday','time_id','passenger_id'],['start_district_hash','Weekday','time_id'],'Weekday_30Min_AcceptOrder_')
        swap_start_dest(b)
        mother_table=distinct_passenger_attribute(b,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_30Min_Desti_AllOrder_')
        swap_start_dest(b)
        swap_start_dest(b_not_null)
        mother_table=distinct_passenger_attribute(b_not_null,mother_table,['Weekday','time_id','passenger_id'],['Weekday','time_id'],'Weekday_30Min_Desti_AcceptOrder_')
        swap_start_dest(b_not_null)
    
    
    
		#去除重复等处理  只统计order supply gap
		a_deduped=a.groupby(['passenger_id','start_district_hash','dest_district_hash','Hour','Minute','Weekday','time_id']).count().reset_index()
		a_deduped=a_deduped.rename(columns={'driver_id':'Supply_cnt_deduped_Weekday','order_id':'Order_cnt_deduped_Weekday'})
		a_deduped['Gap_cnt_deduped_Weekday']=a_deduped.Order_cnt_deduped_Weekday-a_deduped.Supply_cnt_deduped_Weekday
		a_deduped=a_deduped.groupby(['start_district_hash','Weekday','time_id']).sum().reset_index()
		a_deduped=a_deduped['start_district_hash','Weekday','time_id','Order_cnt_deduped_Weekday','Supply_cnt_deduped_Weekday','Gap_cnt_deduped_Weekday']
		
		mother_table=pd.merge(mother_table,a_deduped,on=['start_district_hash','Weekday','time_id'],how='left')
		...
		for now_time_id in range(43,145):
			...
			temp_table.loc[:,['time_id']]=now_time_id
			b=b.append(temp_table)
		b=b.rename(columns={'Order_cnt_deduped_Weekday':'Order_cnt_deduped_Weekday_10min','Supply_cnt_deduped_Weekday':'Supply_cnt_deduped_Weekday_10min','Gap_cnt_deduped_Weekday':'Gap_cnt_deduped_Weekday_10min'})
		mother_table=pd.merge(mother_table,b,on=['start_district_hash','Weekday','time_id'],how='left')
		...
		for now_time_id in range(43,145):
			...
			temp_table.loc[:,['time_id']]=now_time_id
			b=b.append(temp_table)
		b=b.rename(columns={'Order_cnt_deduped_Weekday':'Order_cnt_deduped_Weekday_20min','Supply_cnt_deduped_Weekday':'Supply_cnt_deduped_Weekday_20min','Gap_cnt_deduped_Weekday':'Gap_cnt_deduped_Weekday_20min'})
		mother_table=pd.merge(mother_table,b,on=['start_district_hash','Weekday','time_id'],how='left')
		...
		for now_time_id in range(43,145):
			...
			temp_table.loc[:,['time_id']]=now_time_id
			b=b.append(temp_table)
		b=b.rename(columns={'Order_cnt_deduped_Weekday':'Order_cnt_deduped_Weekday_30min','Supply_cnt_deduped_Weekday':'Supply_cnt_deduped_Weekday_30min','Gap_cnt_deduped_Weekday':'Gap_cnt_deduped_Weekday_30min'})
		mother_table=pd.merge(mother_table,b,on=['start_district_hash','Weekday','time_id'],how='left')	

        #calculate number of gap decreased by dedupe
        for base_name in ['Order_cnt','Supply_cnt','Gap_cnt']:
            for suffix in ['Weekday','Weekday_10min','Weekday_20min','Weekday_30min']:
                cname=base_name+'_deduped'+'_'+suffix
                new_cname=cname+'_duplicate'
                mother_table[new_cname]=-mother_table[suffix+'_'+base_name]-mother_table[cname]
